...
const getGroqClient = () => {
    if (!process.env.GROQ_API_KEY) {
        throw new Error("GROQ_API_KEY Environment Variable missing hai. Render Dashboard/env check karein.");
    }
    return new Groq({ apiKey: process.env.GROQ_API_KEY });
};
...
export const transcribeAudio = async (filePath, language = 'ur') => {
    try {
        if (!fs.existsSync(filePath)) {
            throw new Error("Audio file temporary storage par nahi mili.");
        }
